...
export default function App() {
  ...
  const handleExportLetterPDF = async () => {
    ...
    // Add large bottom padding for clean page breaks
    const originalPaddingBottom = input.style.paddingBottom;
    // Use moderate scale for sharp but lightweight export
    const canvas = await html2canvas(input, { scale: 4, useCORS: true, backgroundColor: null });
    input.style.paddingBottom = originalPaddingBottom; // Restore original padding
    const imgData = canvas.toDataURL('image/png', 0.92); // Good quality, smaller size
    const pdf = new jsPDF({
      orientation: 'p',
      unit: 'pt',
      format: 'a4',
    ...
    // Render image at native canvas width for sharpness
    const imgWidth = canvas.width * (pageWidth / canvas.width); // scale to fit page width
    ...
    const safeFullName = letterFields.fullName ? letterFields.fullName.trim().replace(/\s+/g, ' ') : '';
    const fileName = `Newcommer letter 1${safeFullName ? ': ' + safeFullName : ''}.pdf`;
    pdf.save(fileName);
    setExportState('done');
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 32, alignItems: 'flex-start', padding: 32 }}>
            ...
            <div style={{ flex: 2, minWidth: 320 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginTop: 16 }}>
                <button
                  style={{ padding: '8px 24px', fontSize: 16, borderRadius: 6, background: '#646cff', color: '#fff', border: 'none', cursor: exportState === 'loading' ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
                  onClick={handleExportLetterPDF}
                  disabled={exportState === 'loading'}
                >
                  {exportState === 'loading' && (
                    <span style={{ display: 'inline-block', width: 18, height: 18, border: '2px solid #fff', borderTop: '2px solid #646cff', borderRadius: '50%', animation: 'spin 1s linear infinite', marginRight: 8 }} />
                  )}
